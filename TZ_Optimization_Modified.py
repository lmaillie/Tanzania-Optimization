...
if os.path.isfile(Fexport):
    print("Loading Interim final graph")
    F = nx.read_gpickle(Fexport)
else:
    F = nx.Graph() # F will be graph of just wc and hf

#load the list done
if os.path.isfile(list_done_file):
    print("Opening list of ones completed")
    with open (list_done_file, 'rb') as fp:
        list_done = pickle.load(fp)
else:
    list_done = []

for a in health_facilities[:1]:
    count = 0
    if a in list_done:
        print("Already Done")
    else:
        for b in ward_centers:
            ...
            try:
                ...
                print(count,"/",len(ward_centers), a, b)
                pass
            except:
                pass
                print(count,"/",len(ward_centers),": failed")
        print("Finished WC: ", a)
        list_done.append(a)
        print("saving temp files")
#        nx.write_gpickle(F,Fexport)
#        with open(list_done_file, 'wb') as fp:
#            pickle.dump(list_done, fp)